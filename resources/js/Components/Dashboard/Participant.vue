...
const removeParticipant = async (user) => {
    ...
    const confirmed = await confirm('Are you sure you wanted to remove this participant? All data regarding this participant will be removed.');
    ...
    <div class="list-group-item rounded-0 p-3" :class="data.blocked && 'bg-danger-light'">
        <div class="d-flex align-items-center gap-3">
            ...
            <div class="flex-grow-1">
                ...
                <p class="text-muted small mb-0">Joined on {{ data.created }}</p>
            </div>
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu">
                    ...
                    <li v-if="room.role === 'teacher'">
                        <Link v-if="data.moderator" :href="route('dashboard.classroom.people.moderation', room.code)"
                            class="dropdown-item text-danger" method="post" as="button"
                            :data="{ user: data.id, action: 'remove' }" preserve-scroll>
                        <i class="fas fa-user-minus me-2"></i> Remove Moderator
                        </Link>
                        <Link v-else :href="route('dashboard.classroom.people.moderation', room.code)"
                            class="dropdown-item" method="post" as="button" :data="{ user: data.id, action: 'add' }"
                            preserve-scroll>
                        <i class="fas fa-user-tie me-2"></i> Make Moderator
                        </Link>
                    </li>
                    ...
                    <li v-if="hasPermissions('people.view_activities')">
                        <Link class="dropdown-item" :href="route('dashboard.classroom.people.activities', [room.code, data.id])">
                            <i class="fas fa-chart-line me-2"></i> View Activities
                        </Link>
                    </li>

                    <li v-if="hasPermissions('people.block')">
                        <Link v-if="data.blocked" :href="route('dashboard.classroom.people.action', room.code)"
                        ...
                        <Link v-else :href="route('dashboard.classroom.people.action', room.code)"
                            class="dropdown-item text-danger" method="post" as="button"
                            :data="{ user: data.id, status: 'banned' }" preserve-scroll>
                        <i class="fas fa-ban me-2"></i> Block
                        </Link>
                    </li>

                    <li v-if="hasPermissions('people.remove')">
                        <button @click="removeParticipant(data.id)"
                            class="dropdown-item text-danger" method="post" as="button">
